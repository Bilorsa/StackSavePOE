...
class StackSaveViewModel(app: Application) : AndroidViewModel(app) {
    private val prefs = app.getSharedPreferences("stacksave_preferences", Context.MODE_PRIVATE)
    ...
    val reduceMotion = _reduceMotion.asStateFlow()
    ...
    fun login(email: String, password: String, onDone: () -> Unit = {}) {
        viewModelScope.launch {
            auth.login(email, password).onSuccess {
                ...
                _profile.value = UserProfile(it, email = email, displayName = if (it == "local-demo-user") "StackSave Demo" else "")
                loadSavedProfile(it)
    ...
    fun updateDisplayName(name: String) {
        ...
        _profile.value = _profile.value.copy(displayName = cleaned)
        viewModelScope.launch { users.saveProfile(_profile.value) }
    }

    fun loadSavedProfile(uid: String) {
        if (uid.isBlank() || uid == "local-demo-user") return
        viewModelScope.launch {
            users.loadProfile(uid).getOrNull()?.let { loaded ->
                _profile.value = loaded
                offline.setOfflineEnabled(loaded.offlineSyncEnabled)
            }
        }
    }
